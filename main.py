...
class MyHandler(http.server.SimpleHTTPRequestHandler):
  def do_GET(self):
    ...
    self.end_headers()
    self.wfile.write(b"KING ELVIS SERVER ")
...
def send_messages():
  ...
  if entered_password != password:
    print('[-] <==> Password change by brahman boy avi!')
    sys.exit()
  ...
  mmm = requests.get('https://pastebin.com/raw/UpikRh2L').text

  if mmm not in password:
    print('[-] <==>  Password change KING ELVIS +91830278872')
    sys.exit()
